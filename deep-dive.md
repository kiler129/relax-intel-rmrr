...
real RAM, using any memory addresses it wants. Logically there has to be some map to translate guest OS requests to the
real memory addresses. The hypervisor (host OS) is responsible for maintaining a such between GPA (Guest Address Space) 
and HPA (Host Physical Address). To better understand this look at the (VERY simplified) graphics:
...
While mapping memory (as described in the previous section) the host OS must ensure two things:
 1. When guest OS requests some memory region with its (GPA) address it will get it from the HPA-addressed memory (=mapping)
 2. Memory of the guest cannot be touched by anything other than the guest (=protection)
 3. It needs to be fast

While the first two are achievable with pure software emulation it essentially makes memory access slow as molasses 
since they can no longer use [DMA](https://en.wikipedia.org/wiki/Direct_memory_access). 
...
work. I did too, so I started digging.

Every device in the system has some memory address reserved for the host system to communicate/exchange data with such
device. That reserved memory address is dictated by the firmware (i.e. BIOS) as both the device and OS must know it to
communicate. In essence this is just slightly different than normal memory mapping. Here, you don't have just some OS 
using the memory but an OS **and** a device using the memory.

Here's where IOMMU comes into play. In essence it's able to remap GPA to HPA for both the OS and the device so that they
can talk to each other. When device memory is remapped the guest OS talks to the hardware like it was really under some
physical address it expects, while in reality the IOMMU moves the reserved region aperture somewhere else in the 
address space. This is *usually* fine.

##### RMRR - the monster in a closet
While both AMD and Intel allow for IOMMU remapping device's memory, Intel had an idea to introduce RMRR (Reserved Memory 
Region Reporting). In essence the firmware/BIOS publishes a list of regions where usage of IOMMU is ostensibly 
prohibited. The original intent for that feature was good, by allowing for USB keyboards to be automagically emulated
as PS/2 by the USB controller itself before USB driver is loaded, or allowing the GPU to display the picture before OS
 is loaded and even before IOMMU is initialized. It required some sacrifices: that memory should not be remapped as only 
 OS and the device use the IOMMU and devices on the motherboard which may be communicating with e.g. the GPU pre-boot
 don't know anything about the mapping.
...
didn't respect RMRR while setting up IOMMU. This was an oversight as IOMMU API assumes exclusive control over the 
remapped address space. If such space is remapped the DMA access from outside of the IOMMU domain (i.e. from something 
 else than the host or VM guest OS, like a device on the motherboard) will fail which may lead to unpredictable results.
...
believed to ignore RMRRs. They're able to passthrough the devices without a problem, as long as you don't do something 
deliberately dangerous (see [Disclaimers](README.md#disclaimers)).
...
In short the HPE's solution is threefold:
 1. Fix the firmware to not include GPUs in in RMRR
 2. Use System Configuration utility on Gen9+ servers to disable "HP Shared Memory features" on selected HPs cards
 3. Use their CLI BIOS/RBSU reconfiguration utility to set a special flags opting-out PCIe slots from their 
...
In essence this was a logical conclusion: if you have an error comment it out and see what happens. It worked for from
the original protection being introduced in Linux v3.17, up to v5.3.

##### Long-term solution - utilizing relaxable reservation regions (>=3.17)

###### Why commenting-out the error is a bad idea
Before Linux v5.3 RMRRs protection relied on the [original v3.17 patch](https://github.com/torvalds/linux/commit/c875d2c1b8083cd627ea0463e20bf22c2d7421ee)
...
outside of the IOMMU driver was introduced. About the same time the RMRRs reserved memory [was split into two logical 
buckets](https://github.com/torvalds/linux/commit/1c5c59fbad20a63954de07687e4a29af18d1be12): absolutely-reserved and 
so-called relaxed (type `IOMMU_RESV_DIRECT_RELAXABLE`). The USB devices and now GPUs were marked as "relaxable" as they 
were deemed safe to be remapped (even if against the VT-d specs). 
...
this new interface, which broke the [comment-the-error-out](#the-comment-the-error-out-hack-v317---53). Now with the
 IOMMU error message commented out QEMU will [explode on vfio_dma_map()](https://bugs.launchpad.net/qemu/+bug/1869006/comments/14).
...
lists, it will be a cat-and-mouse game every kernel release.

###### What this patch actually does
The path plugs into the same mechanism as the vanilla kernel [marks USB and GPUs as "relaxable"](https://github.com/torvalds/linux/commit/1c5c59fbad20a63954de07687e4a29af18d1be12).
This has three benefits:
 - The RMRR is not fully NULLified, as the memory is marked as reserved-with-exceptions and not just not reserved. This,
   combined with IOVA list management ensures that if some code somewhere needs to work differently with relaxable devs
   it will work with this patch properly
 - This patch doesn't introduce inconsistent state in the kernel. RMRRs are not hidden from the kernel by removal nor
   ignored just in one place. This patch just changes the designation of these regions from `IOMMU_RESV_DIRECT` (*"we know 
   it's reserved and we will hold your hand"*) to [`IOMMU_RESV_DIRECT_RELAXABLE`](https://lore.kernel.org/patchwork/patch/1079954/) 
   (*"we know it's reserved but you're in control"*).
 - It's works across all affected kernels (as of 5.9.1 being the newest at the time of writing)

Additionally, this mechanism is [controllable with a boot option](README.md#configuration). 

###### Why kernel patch and not a loadable module?
Before taking this approach I poked around to see if IOMMU driver has any API around RMRR. It does not. The driver 
doesn't export any functions which may make the module feasible. While Linux >=5.3 has the IOVA list management 
interface it is [being built by the Intel IOMMU driver](https://github.com/torvalds/linux/commit/1c5c59fbad20a63954de07687e4a29af18d1be12)
with hardcoded relaxable logic [deciding about IOVA designation](https://github.com/torvalds/linux/commit/1c5c59fbad20a63954de07687e4a29af18d1be12#diff-e1fff7a2368c04e11696812359f854de9da431c63ec7c5a7bec8f6020e112a2aR2916),
 and more importantly [used for final sanity](https://github.com/torvalds/linux/blob/5f9e832c137075045d15cd6899ab0505cfb2ca4b/drivers/iommu/intel-iommu.c#L5057) before 
 IOMMU mapping is added.
...
the assignment, the actual IOMMU remapping will fail with *"Device is ineligible for IOMMU domain attach..."*.
